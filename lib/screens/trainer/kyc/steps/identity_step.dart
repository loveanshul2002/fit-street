...
class IdentityStep extends StatelessWidget {
  ...
  static bool validateAge(
      TextEditingController dobCtrl, void Function(String) toast) {
    final txt = dobCtrl.text.trim();
    final parts = txt.split("/");
    if (parts.length != 3) {
      toast("Enter DOB as DD/MM/YYYY");
      return false;
    }
    ...
    if (d == null || m == null || y == null) {
      toast("Enter a valid DOB");
      return false;
    }
    final now = DateTime.now();
    final dobDate = DateTime(y, m, d);
    final ageYears = now.year -
        dobDate.year -
        ((now.month < m || (now.month == m && now.day < d)) ? 1 : 0);
    if (ageYears < 18) {
  ...
  Future<void> _pickImageAndSend(
      BuildContext ctx, Future<void> Function(String) sendCallback) async {
    try {
  ...
  Widget _imageRow(
      String label, String? path, VoidCallback onPick, VoidCallback onRemove,
      {bool required = false}) {
    return Padding(
          ...
          child: Row(
            children: [
              if (path == null || path.isEmpty) ...[
                ...
                Expanded(
                    child: Text("Tap to choose photo",
        ...
        child: SingleChildScrollView(
          child:
            ...
            field("Full Name", fullName,
                validator: req, readOnly: readOnlyFullName),
            ...
            // Bio Data
            field("Bio (about you)", bioData, maxLines: 4, validator: (v) {
            ...
            SwitchListTile(
                ...
                title: const Text("Same as permanent address",
                    style: TextStyle(color: Colors.white))),

            // If not same, show CURRENT pincode + city/state + address
            if (!sameAsPermanent) ...[
              const SizedBox(height: 8),
              field("Current Pincode", currentPincode,
                  keyboardType: TextInputType.number,
                  maxLength: 6,
                  inputFormatters: [
                    FilteringTextInputFormatter.digitsOnly,
            ...
            field("Aadhaar (XXXX XXXX XXXX)", aadhaar,
                keyboardType: TextInputType.number,
                inputFormatters: [
                  AadhaarFormatter(),
                ...
                validator: (v) =>
                    RegExp(r'^\d{12}$').hasMatch((v ?? "").replaceAll(" ", ""))
                        ? null
            ...
            _imageRow(
              ...
              () => _pickImageAndSend(
                  context, (path) async => pickAadhaarPhotofront(path)),
            ...
            _imageRow(
              ...
              () => _pickImageAndSend(
                  context, (path) async => pickAadhaarPhotoback(path)),
