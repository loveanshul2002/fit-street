...
class _FindTrainersScreenState extends State<FindTrainersScreen> {
  ...
  static const Set<String> _allowedtrainerSpecs = {
    'strength training',
    'hiit',
    'crossfit',
    'functional training',
    ...
    'mobility and flexibility coach',
  ...
  void initState() {
    ...
    _groupedSpecs =
        _getFilteredGroupedSpecializations(widget.initialCategory ?? 'Trainer');
    _boot();
  }
  ...
  Future<void> _getTrainers() async {
    ...
    try {
      ...
      // If no explicit specialization picked, use initialCategory (Angular parity); default to Trainer
      ...
      if (resp.statusCode == 200) {
        ...
        // Filter to only allowed Trainer specializations (normalize '&' -> 'and', collapse spaces, lowercase)
        String _norm(String s) => s
            .toLowerCase()
            .replaceAll('&', 'and')
            .replaceAll(RegExp(r'\s+'), ' ')
            .trim();
        final normalizedAllowed = _allowedtrainerSpecs.map(_norm).toSet();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Find Fitness Trainers'),
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            '${_totalCount > 0 ? _totalCount : _filtered.length}  Fitness Trainers experts available in ${_locationCtrl.text.trim().isNotEmpty ? _locationCtrl.text.trim() : 'your area'}',
                            style: const TextStyle(
                                color: Colors.white,
                                fontSize: 16,
